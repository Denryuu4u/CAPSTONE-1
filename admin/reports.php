...
<head>
    ...
    <style>
        ...
        @media print {
            ...
            .rp-signature    { page-break-inside: avoid; break-inside: avoid; }
        ...
        /* Signature block — bottom of each report, signed after printing. */
        .rp-signature { margin-top:36px; display:flex; justify-content:flex-end; }
        .rp-sign-box { width:280px; text-align:center; }
        .rp-sign-line { border-top:1.6px solid #1f2937; margin-top:46px; }
        .rp-sign-name { font-weight:700; font-size:0.8rem; color:#111827; margin-top:6px; }
        .rp-sign-sub  { font-size:0.6rem; color:#9ca3af; margin-top:2px; text-transform:uppercase; letter-spacing:.04em; }
    ...
    return `<div class="rp-header">
        ...
        <div class="rp-title-block">
            ...
            <div class="rp-meta">
                ...
                <strong>Period:</strong> ${getDateRange()}
            </div>
        </div>
    </div>`;
}

// Signature block — sits at the very bottom of each report so it can be signed
// after printing.
function rpFooter(){
    return `<div class="rp-signature">
        <div class="rp-sign-box">
            <div class="rp-sign-line"></div>
            <div class="rp-sign-name"><?= htmlspecialchars($user_name) ?></div>
            <div class="rp-sign-sub">Generated by · Signature over Printed Name / Date</div>
        </div>
    </div>`;
}
...
function buildProject(cat){
    ...
    return `<div class="rp-doc">${rpHeader('Project Report'+(cat?' — '+(CAT_LABEL[cat]||cat):''))}
        ...
        <div class="rp-summary"><div class="rp-summary-title">Summary</div><div class="rp-summary-grid">${sum}</div></div>${rpFooter()}</div>`;
}
...
function buildCutting(cat){
    ...
    return `<div class="rp-doc">${rpHeader('Cutting List Summary'+(cat?' — '+(CAT_LABEL[cat]||cat):''))}
        ...
        <div class="rp-summary"><div class="rp-summary-title">Summary – Total per Category</div><div class="rp-summary-grid">${sum}</div></div>${rpFooter()}</div>`;
}
